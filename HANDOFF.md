...
| Tadabbur mode (P20, T30) | lamp button in the reader's bottom bar → gathering: each tap adds an ayah (consecutive or scattered, across pages), a second tap removes it; "حفظ في بطاقة" → new card (title, themes) or an existing one. Card sheet: title, its ayahs each removable (undo), "add ayahs from the mushaf", themes, note with guiding questions, tafsir. Journal at `/tadabbur`: cards filtered by theme, 30-juz map, diacritic-insensitive search, copy as text, manage themes. Tadabbur reading is separate from the khatma (own position, own time: top bar + journal tiles and 7-day bars). Desktop opens two facing pages (P6). Synced as `users/{uid}/profile/tadabbur` | done, unit-tested (`tadabbur.spec.ts`) — **needs the rules deployed** to sync |
| First-run guidance (P21) | `core/onboarding` + `ui/welcome.ts` (3 skippable cards, first open only, not shown to someone who already has readings) + `ui/tip.ts` one-time tips in the reader and on first tadabbur; "جولة التطبيق" in More / sidebar replays all | done, unit-tested (`onboarding.spec.ts`) |
